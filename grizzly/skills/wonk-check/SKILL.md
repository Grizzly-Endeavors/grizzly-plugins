...
1. **What.** Quote or reference the specific lines. Keep it tight.
2. **Category.** Which wonk category it falls into (can be more than one).
3. **Why it's wonky.** One or two sentences explaining what's suspicious
   about it. Be specific — "this is complex" is not useful. "This maps
   a Vec<User> to a Vec<User> with identical fields" is.
4. **Suggested fix.** What the code would look like without the wonk.
   Keep this concrete — a sketch or one-liner, not a lecture.

## Severity & Tone

Not all wonk is equal. Use this scale, but don't be rigid about it:
...
The tone should be collegial code review, not a lecture. Think "hey, is
there a reason for this?" not "this is wrong and bad." The whole point
of wonk-check is that the code *works* — the question is whether it
needs to work *this way*.
...
If the file is clean, say so. "No wonk found" is a valid and good
outcome. Don't manufacture findings.